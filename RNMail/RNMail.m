                  ...
                  callback: (RCTResponseSenderBlock)callback)
{
    if ([MFMailComposeViewController canSendMail])
    {
        ...
        NSArray *attachmentArray = options[@"attachment"];
        if(attachmentArray != nil && attachmentArray.count> 0 ){
            for (NSDictionary *attachmentData in attachmentArray){
                
                if (attachmentData[@"path"] && attachmentData[@"type"]){
                    NSString *attachmentPath = [RCTConvert NSString:attachmentData[@"path"]];
                    NSString *attachmentType = [RCTConvert NSString:attachmentData[@"type"]];
                    NSString *attachmentName = [RCTConvert NSString:attachmentData[@"name"]];
                    ...
                    NSError* error = nil;
                    NSURL *fileURL = [NSURL fileURLWithPath:attachmentPath];
